...
// import personifyinc api
const personifyincApi = require("./personifyinc-api")
...
scimgateway.createUser = async (baseEntity, userObj) => {
  ...
  try {
    ...
    // call personifyinc api create user
    await personifyincApi.apiCreateUser(pluginName, baseEntity, scimgateway, userObj)
...
scimgateway.modifyUser = async (baseEntity, id, attrObj) => {
  ...
  if (attrObj["active"] !== undefined) {
    if (attrObj["active"] === false) {
      // call personifyinc api delete user
      await personifyincApi.apiDeleteUser(pluginName, baseEntity, scimgateway, "", id)
    } else {
      // call personifyinc api create user
      await personifyincApi.apiCreateUser(pluginName, baseEntity, scimgateway, userObj)
    }
  } else {
    // call personifyinc api update user
    await personifyincApi.apiUpdateUser(pluginName, baseEntity, scimgateway, userObj, id)
  }
